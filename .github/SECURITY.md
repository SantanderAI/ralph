...
- The third-party AI coding CLIs that `ralph` invokes (`codex`, `claude`, `gemini`, `devin`) — report those to their respective maintainers
- Third-party dependencies (report those to the respective maintainers)

`ralph` is a dependency-free Bash / PowerShell loop wrapper. It launches the AI
CLIs you already have installed, sources configuration from `.ralph/.env`, and
writes logs to `.ralph/logs/`. Reports concerning command injection through
configuration values, unsafe handling of `.ralph/.env`, path traversal in log
rotation, or sandbox/RAM-limit escape are in scope.
...
- Treat `.ralph/.env` as local-only configuration; never commit a populated `.env`
- Keep dependencies up to date (Dependabot updates are configured in `.github/dependabot.yml`)
